  ...
  useEffect(() => {
    api.get("/movies").then((response) => {
      const sortedMovies = response.data.sort(
        (a, b) => b.vote_average_user - a.vote_average_user
      );
    ...
    try {
      ...
      if (movieData && user) {
        ...
        if (ratingUser) {
          ...
          const userRatings =
            JSON.parse(localStorage.getItem("userRatings")) || {}; // Lấy giá trị userRatings từ localStorage, nếu không có thì tạo một đối tượng mới
                      ...
                      <div>
                        ...
                        {movie.vote_count_user !== 0 ? (
                          <span>
                            ...
                            &nbsp; bởi <b>{movie.vote_count_user}</b> người
                            dùng)
                          </span>
                        ) : (
